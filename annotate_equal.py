"""
equal画像でフェーダー位置を検索・アノテーションするスクリプト。
"""
from PIL import Image, ImageDraw
import numpy as np

img = Image.open("/home/ubuntu/mixer_screenshot_equal.png")
arr = np.array(img)
...
# 最初のフェーダー（Track1）と最後のフェーダー（MASTER）のY位置
if len(fader_ys) >= 2:
    ...
    draw.text((5, track_y - 18), f"Track Handle: Y={track_y}", fill=(255, 50, 50))
    draw.text((5, master_y + 5), f"MASTER Handle: Y={master_y}", fill=(255, 220, 0))
    if track_y != master_y:
        draw.text((5, (track_y+master_y)//2), f"Diff: {track_y-master_y}px", fill=(255,255,255))
    else:
        draw.text((5, track_y + 20), "ALIGNED!", fill=(0, 255, 0))

img.save("/home/ubuntu/mixer_annotated_equal.png", "PNG")
print("保存: /home/ubuntu/mixer_annotated_equal.png")
