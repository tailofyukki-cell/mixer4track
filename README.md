...
### Phase 23.1: ファイル読込・トランスポートレイアウト修正
- **ファイル読込時クラッシュを修正**。マーカーUI更新が参照していた欠落API `get_track_duration_sec()` を互換APIとして復元。
- **トラック列に横幅を再配分**。表示中の8トラックが利用可能な横幅を均等に使うようにし、MASTER右側の不要な余白を解消。
